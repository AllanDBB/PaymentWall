...
VALUES
    ...
    ('Rio de Janeiro', 9),
    ('Mumbai', 10),
    ('San José', 11);  -- Capital de Costa Rica añadida

SELECT * FROM PW_cities;


ALTER TABLE PW_currency MODIFY name VARCHAR(100) NOT NULL;
...
FROM (
    SELECT 
        u.user_id, 1 + FLOOR(RAND() * 10) AS permission_id 
    FROM (
...
SELECT * FROM PW_userPermissions;

-- permissions for roles
SET @row = (SELECT IFNULL(MAX(rolePermissionId), 0) FROM PW_rolePermissions);

INSERT INTO PW_rolePermissions (rolePermissionId, enabled, deleted, lastUpdate, userName, checksum, permissionId, roleId)
SELECT 
    (@row := @row + 1) AS rolePermissionId, 
    1 AS enabled,
    0 AS deleted,
    NOW() AS lastUpdate,
...
FROM 
    PW_Payments p
JOIN
    (SELECT exchangeRateId FROM PW_exchangeRate ORDER BY RAND()) er
CROSS JOIN 
...
SELECT 
    CASE 
        WHEN n = 1 THEN 'Fallo en predicción de IA - Sesgo detectado en modelo'
        WHEN n = 2 THEN 'IA generó recomendación inválida para usuario'
        WHEN n = 3 THEN 'Tiempo de respuesta de IA excedido'
        WHEN n = 4 THEN 'Error en carga de modelo de machine learning'
        WHEN n = 5 THEN 'IA produjo falso positivo en detección de fraude'
        WHEN n = 6 THEN 'IA produjo falso negativo en detección de fraude'
        WHEN n = 7 THEN 'Problema de conectividad con servicio de IA'
        WHEN n = 8 THEN 'Error en preprocesamiento de datos para IA'
        WHEN n = 9 THEN 'Sesgo racial detectado en resultados de IA'
        WHEN n = 10 THEN 'Modelo de IA necesita reentrenamiento'
    END AS description,
    NOW() - INTERVAL FLOOR(RAND() * 30) DAY - INTERVAL FLOOR(RAND() * 24) HOUR AS posttime,
    CONCAT('AI-SRV', FLOOR(1 + RAND() * 3)) AS computer,
    CASE 
        WHEN RAND() > 0.7 THEN CONCAT('user', FLOOR(1 + RAND() * 40)) 
        ELSE 'ai-engine' 
    END AS username,
    CONCAT('AI-ERR-', FLOOR(RAND() * 9000) + 1000) AS trace,
    CASE WHEN RAND() > 0.5 THEN FLOOR(1 + RAND() * 40) ELSE NULL END AS referenceId1,
    CASE 
        WHEN n = 1 THEN CONCAT('Sesgo detectado en categoría: ', ELT(FLOOR(1 + RAND() * 5), 'género', 'edad', 'ubicación', 'historial', 'comportamiento'))
        WHEN n = 2 THEN CONCAT('Usuario: user', FLOOR(1 + RAND() * 40), ', Recomendación inválida: ', ELT(FLOOR(1 + RAND() * 5), 'producto no disponible', 'precio incorrecto', 'categoría errónea', 'restricción ignorada', 'conflicto detectado'))
        WHEN n = 3 THEN CONCAT('Tiempo de respuesta: ', FLOOR(10 + RAND() * 20), 's (límite: 5s)')
        WHEN n = 4 THEN CONCAT('Modelo: ', ELT(FLOOR(1 + RAND() * 3), 'fraude-v3', 'recomendaciones-v2', 'clasificación-v1'), ', Error: ', ELT(FLOOR(1 + RAND() * 3), 'pesos corruptos', 'versión incompatible', 'formato inválido'))
        WHEN n = 5 THEN CONCAT('Transacción ID: ', FLOOR(RAND() * 100000), ', Confianza: ', ROUND(RAND() * 100, 2), '%')
        WHEN n = 6 THEN CONCAT('Transacción ID: ', FLOOR(RAND() * 100000), ', Confianza: ', ROUND(RAND() * 100, 2), '%')
        WHEN n = 7 THEN CONCAT('Servicio: ', ELT(FLOOR(1 + RAND() * 3), 'tensorflow-serving', 'sagemaker', 'custom-api'), ', Tiempo caído: ', FLOOR(5 + RAND() * 30), 'min')
        WHEN n = 8 THEN CONCAT('Campo problemático: ', ELT(FLOOR(1 + RAND() * 5), 'edad', 'ubicación', 'historial', 'monto', 'frecuencia'), ', Error: ', ELT(FLOOR(1 + RAND() * 3), 'valor nulo', 'formato inválido', 'fuera de rango'))
        WHEN n = 9 THEN CONCAT('Métrica afectada: ', ELT(FLOOR(1 + RAND() * 3), 'precisión', 'recall', 'f1-score'), ', Diferencia: ', ROUND(10 + RAND() * 30, 2), '%')
        WHEN n = 10 THEN CONCAT('Precisión actual: ', ROUND(70 + RAND() * 20, 2), '%, Precisión mínima requerida: 90%')
    END AS value1,
...
-- Verificación de logs de IA insertados
SELECT 
    logId, 
    posttime, 
    description, 
...
FROM
    (SELECT eventId, eventTypeId FROM PW_eventByAI LIMIT 50) e
JOIN 
    (SELECT contactInfoId FROM PW_contactInfo WHERE contactInfoId IS NOT NULL LIMIT 100) ci ON 1=1
JOIN 
    (SELECT userId FROM PW_users WHERE userId IS NOT NULL LIMIT 100) u ON 1=1
JOIN 
    (SELECT transactionId FROM PW_transactions WHERE transactionId IS NOT NULL LIMIT 100) t ON 1=1
JOIN 
    (SELECT paymentId FROM PW_Payments WHERE paymentId IS NOT NULL LIMIT 100) p ON 1=1
JOIN 
    (SELECT addressId FROM PW_address WHERE addressId IS NOT NULL LIMIT 100) a ON 1=1
JOIN 
    (SELECT companyAddressId FROM PW_companyAddress WHERE companyAddressId IS NOT NULL LIMIT 100) ca ON 1=1
CROSS JOIN 
    (SELECT @row := IFNULL((SELECT MAX(interactionId) FROM PW_interactionByAI), 0)) r
WHERE
    e.eventId IS NOT NULL
    AND ci.contactInfoId IS NOT NULL
    AND u.userId IS NOT NULL
    AND t.transactionId IS NOT NULL
    AND p.paymentId IS NOT NULL
    AND a.addressId IS NOT NULL
    AND ca.companyAddressId IS NOT NULL
LIMIT 100;
...
FROM 
    PW_Payments p
JOIN 
    (SELECT currencyId FROM PW_currency ORDER BY RAND()) c
JOIN
    (SELECT exchangeRateId FROM PW_exchangeRate ORDER BY RAND()) er
CROSS JOIN 
    (SELECT 1 UNION SELECT 2 UNION SELECT 3) AS multiplier
